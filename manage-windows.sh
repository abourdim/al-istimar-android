#!/usr/bin/env bash
# ============================================================================
#  Al Istimar Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/13-al-istimar}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.alistimar}"
AVD_NAME="${AVD_NAME:-al_istimar_test}"
...
cmd_test() {
  ...
  local out="/tmp/al-istimar-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Al Istimar Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
